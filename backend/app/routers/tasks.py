...
from app.schemas.task_validation import (
    TaskValidationIssue,
...
def _validate_tasks(tasks: list[dict[str, Any]]) -> tuple[list[TaskValidationIssue], set[int]]:
    ...
    for idx, task in enumerate(tasks):
        ...
        if not _is_non_empty_string(task.get("title")):
            mark_invalid(idx, task, "title must be a non-empty string.")
        if not _is_non_empty_string(task.get("prompt")):
        ...
        if not isinstance(responses, list):
            ...
            for r_idx, response in enumerate(responses):
                if not isinstance(response, dict):
                    ...
                    continue
                if not _is_non_empty_string(response.get("label")):
                ...
                if model is not None and not isinstance(model, str):
                    mark_invalid(idx, task, f"responses[{r_idx}].model must be a string when provided.")
        ...
        if not isinstance(dimensions, list) or len(dimensions) == 0:
            ...
            for d_idx, dimension in enumerate(dimensions):
                ...
                if not _is_non_empty_string(dimension.get("description")):
                    mark_invalid(
                        idx,
                        task,
...
async def validate_tasks(body: TaskValidationRequest) -> TaskValidationResponse:
    issues, invalid_rows = _validate_tasks(body.tasks)
